        ...
        <div class="container">
            <div class="flex justify-center">
                <div class="w-full px-4 text-center">
                    <div>
                        <a href="/tutorials" class="text-xl text-gray-500 mx-4 hover:text-primary hover:font-bold">Semua</a>
                        @php
                            $categories = App\Models\Category::all();
                        @endphp
                        @foreach ($categories as $category)
                        <a href="/categories/{{ $category->slug }}" class="text-xl text-gray-500 mx-4 hover:text-primary hover:font-bold">{{ $category->name }}</a>
                        @endforeach
                    </div>
                </div>
            </div>
        ...
        <div class="container">
            <div class="flex flex-wrap justify-start">
                ...
                <div class="w-full md:w-1/4 relative px-4 mb-8">
                    <a href="/categories/{{ $tutorial->category->slug }}" class="absolute top-0 py-1 text-secondary px-3 bg-orange-500 rounded-r rounded-t-none shadow">{{ $tutorial->category->name }}</a>
                        <a href="" class="w-full">
                            <div class="rounded shadow-md overflow-hidden">
                                <img src="{{ asset('images/rajartan.png') }}" alt="">
